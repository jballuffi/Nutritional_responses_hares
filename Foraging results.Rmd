...
After incorporating food treatment into the top performing model (D2), results partially changed. Mainly, we found no effect of available biomass on foraging effort cross both controls and food supplemented individuals (t = `r foodsb_t`; Figure 4C). However, in line with the top control model, we found that foraging effort increased `r foodt_coef*10*60` ± `r foodt_se*10*60` minutes per day for every 10 kg increase in available biomass (t = `r foodt_t`; Figure 4D). Overall, food supplemented individuals foraged `r abs(food_coef*60)` ± `r food_se*60` minutes less than controls, but this effect was not significant (t = `r food_t`).
