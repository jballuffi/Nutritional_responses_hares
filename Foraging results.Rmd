...
The most parsimonious model for explaining foraging efforts of control hares (delta AIC < 2) was that which included twig biomass and temperature as fixed effects (Table 1, model D2). This model found that hares foraged `r sb_coef*10*60` ± `r sb_se*10*60` minutes more per day for every 10 kg per hectare increase in available twig biomass (t = `r sb_t`). This meant that as twig biomass increased from `r sb_pred[, min(x)]` (minimum observed) to `r sb_pred[, max(x)]` kg/ha (maximum observed), hares foraging effort increased from `r sb_pred[x == min(x), round(predicted, 1)]` to `r sb_pred[x == max(x), round(predicted, 1)]` hr/day (Figure 3A). Additionally, the same model showed that for every 10 degree C increase in ambient temperature, hares foraged `r t_coef*10*60` ± `r t_se*10*60` minutes more per day (t = `r t_t`). Meaning that as weekly temperatures increased from the coldest of `r t_pred[, min(x)]` to the warmest of `r t_pred[, max(x)]`, hare foraging rate increased from `r t_pred[x == min(x), round(predicted, 1)]` to `r t_pred[x == max(x), round(predicted, 1)]` hours per day (Figure 3B).
